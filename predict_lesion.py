...
from torch import load, cat, no_grad, where
from torch.utils.data import Dataset, DataLoader
from albumentations import Resize, Compose, ToFloat, Normalize
from albumentations.pytorch import ToTensorV2
from torchvision.utils import save_image
from torchvision import transforms
from torch.nn import Module, ReLU, Conv2d, BatchNorm2d, MaxPool2d, Upsample
import cv2
import time
...
IMAGENET_WEIGHTS = {
    ...
    'efficientnet-b7': 'https://github.com/lukemelas/EfficientNet-PyTorch/releases/download/1.0/efficientnet-b7-dcc49843.pth', #'efficientnet-b7-dcc49843.pth',
}
...
class UNet(Module):
    ...
    def forward(self, input):
        ...
        x3_1 = self.conv3_1(cat([x3_0, self.up(x4_0)], 1))
        x2_2 = self.conv2_2(cat([x2_0, self.up(x3_1)], 1))
        x1_3 = self.conv1_3(cat([x1_0, self.up(x2_2)], 1))
        x0_4 = self.conv0_4(cat([x0_0, self.up(x1_3)], 1))
...
class retinaDataset(Dataset):
    def __init__(self, image, transform = None ): #image_dir, transform=None):
        #self.image_dir = image_dir
        #self.mask_dir = mask_dir
        self.transform = transform
        self.images = image #os.listdir(image_dir)

    def __len__(self):
        return len(self.images)

    def __getitem__(self, index):
        image = cv2.imread(self.images)
...
def test_result(model, dataloaders, device = 'cpu'):
    model = '' 
    with no_grad():
        for idx, (image) in enumerate(dataloaders):
            image = image.to(device)

            preds = model(image)
            y_pred = preds.cpu().numpy()
            
            save_image(preds, 'HE.png')
...
def predict():
    ...
    input_image = 'input.png' #D:/programing/python/dr_system/test_v02/input.png'
    image_height = 640
    image_width = 640
    ...
    data_transforms = Compose(
            [
                ...
                ToFloat(),
                #Normalize(mean = (0.352, 0.392, 0.392), std = (0.229, 0.224, 0.225)),
                ToTensorV2(),
            ],
    )

    data = retinaDataset(image= input_image, transform= data_transforms)
    dataloaders = DataLoader(data, batch_size = 1 )
     
    eff_ex = 'effunet_FGADR_ex.pth' #'effunet_IDRiD_ex.pth' #'effunet_FGADR_ex_1.pth' 
    eff_he = 'effunet_FGADR_he.pth' #'effunet_IDRiD_he_distortion.pth' #'effunet_FGADR_he_1.pth'
    eff_se = 'effunet_FGADR_se.pth'
    eff_ma = 'effunet_FGADR_ma.pth'

    eff_ex_model = get_efficientunet_b7(out_channels=1, concat_input=True, pretrained = True)
    eff_ex_model.to(device)
    eff_ex_model.load_state_dict(load(eff_ex, map_location = device))    

    eff_he_model = get_efficientunet_b7(out_channels=1, concat_input=True, pretrained = True)
    eff_he_model.to(device)
    eff_he_model.load_state_dict(load(eff_he, map_location = device))

    eff_se_model = get_efficientunet_b7(out_channels=1, concat_input=True, pretrained = True)
    eff_se_model.to(device)
    eff_se_model.load_state_dict(load(eff_se, map_location = device))

    eff_ma_model = get_efficientunet_b7(out_channels=1, concat_input=True, pretrained = True)
    eff_ma_model.to(device)
    eff_ma_model.load_state_dict(load(eff_ma, map_location = device))
    
    """
    model_ex = UNet_(n_channels = 3, n_classes=1, bilinear = 'bilinear') #UNet(num_classes = 1)
    ex = 'unet_IDRiD_ex.pth' #'unet_FGADR_ex.pth' 
    model_ex.to(device)
    model_ex.load_state_dict(load(ex, map_location = device))# , map_location = 'cpu')#.state_dict(), strict= False)

    #net = torch.hub.load('milesial/Pytorch-UNet', 'unet_carvana', pretrained = False)
    #summary(eff_ex_model, input_size = (3, 640, 640), device = 'cpu')
    #summary(net, input_size = (3, 640, 640), device = 'cpu')
    #input()

    
    model_se = UNet_(n_channels = 3, n_classes=1, bilinear = 'bilinear')#UNet(num_classes = 1)
    se = 'unet_FGADR_se.pth' 
    model_se.to(device)
    model_se.load_state_dict(load(se, map_location = device ))#.module.state_dict(), strict= False) #, map_location = 'cpu').module.state_dict(), strict= False)
    
    model_he = UNet_(n_channels = 3, n_classes=1, bilinear = 'bilinear') #UNet(num_classes = 1)
    he = 'unet_FGADR_he.pth' 
    model_he.to(device)
    model_he.load_state_dict(load(he, map_location = device)) #, map_location = 'cpu')#.module.state_dict(), strict= False)

    model_ma = UNet(num_classes = 1)
    ma = 'unet_FGADR_ma.pth' 
    model_ma.to(device)
    model_ma.load_state_dict(load(ma, map_location = device).module.state_dict(), strict= False) #, map_location = 'cpu').module.state_dict(), strict= False)
    """
    print('---예측시작---')
    ...
    with no_grad():
        for idx, image in enumerate(dataloaders):
            ...
            #test = image.squeeze(0).numpy()
            #print(image.shape, image[0].shape)
            #plt.imshow( (np.uint8(np.transpose(image[0].numpy(), (1, 2, 0))*255))) #.astype(np.int32) )
            #plt.show()
            ...
            preds_ex = eff_ex_model(image)    # eff_ex_model(image) # model_ex(image)
            preds_ex = torch.sigmoid(preds_ex)
            preds_ex = where(preds_ex >= 0.25, 1, 0) 
            preds_se = eff_se_model(image)    # eff_se_model(image) # model_se(image)
            preds_se = torch.sigmoid(preds_se)
            preds_se = where(preds_se >= 0.25, 1, 0)
            preds_he = eff_he_model(image)    # eff_he_model(image) # model_he(image)
            preds_he = torch.sigmoid(preds_he)
            preds_he = where(preds_he >= 0.25, 1, 0)
            preds_ma = eff_ma_model(image)    # eff_ma_model(image) # model_ma(image)
            preds_ma = torch.sigmoid(preds_ma)
            preds_ma = where(preds_ma >= 0.25, 1, 0)
            #print('preds;;', preds_ex.shape)

            
            save_image(preds_ex.float(), 'EX.png')
            save_image(preds_se.float(), 'SE.png')
            save_image(preds_he.float(), 'HE.png')
            save_image(preds_ma.float(), 'MA.png')
    ...
    #print('inference? end {}m {}s'.format(end//60, end%60))
    print('inference? end {}m {}s'.format(end_time//60, end_time%60))


"""
class lesion_predict(QThread):
    device = 'cpu'
    input = 'input.png'
    image_height = 1280
    image_width = 1280
    ...
    data = retinaDataset(image= input, transform= data_transforms)
    dataloaders = DataLoader(data, batch_size = 1)
    print('dataloaders;', len(dataloaders))

    model_ex = UNet(num_classes = 1)
    ex = 'unet_FGADR_ex.pth' 
    model_ex.load_state_dict(load(ex).module.state_dict(), strict= False)
    #model_ex = load(open(ex, 'rb'))
    #model_ex.eval()


    model_se = UNet(num_classes = 1)
    se = 'unet_FGADR_se.pth' 
    model_se.load_state_dict(load(se).module.state_dict(), strict= False)
    #model_se = load(open(ex, 'rb'))
    #model_se.eval()

    model_he = UNet(num_classes = 1)
    he = 'unet_FGADR_he.pth' 
    model_he.load_state_dict(load(he).module.state_dict(), strict= False)
    #model_he = model.load_state_dict(torch.load(model_he), strict = False)
    #model_he.eval()

    model_ma = UNet(num_classes = 1)
    ma = 'unet_FGADR_ma.pth' 
    model_ma.load_state_dict(load(ma).module.state_dict(), strict= False)
    #model_ma.eval()
    ...
    with no_grad():
        for idx, image in enumerate(dataloaders):
            ...
            image = image.to(device)
            print('image ;;', type(image), image.shape)
            preds_ex = model_ex(image)    
            preds_se = model_se(image)
            preds_he = model_he(image)    
            preds_ma = model_ma(image)    

            save_image(preds_ex, 'EX.png')
            save_image(preds_se, 'SE.png')
            save_image(preds_he, 'HE.png')
            save_image(preds_ma, 'MA.png')

            break

    end = time() - start
    print('ex end {}m {}s'.format(end//60, end%60))
    ...
    with no_grad():
        for idx, image in enumerate(dataloaders):
            ...
            image = image.to(device)
            preds = model_se(image)    

            save_image(preds, 'SE.png')
            break

    end = time() - start
    print('se end {}m {}s'.format(end//60, end%60))

    start = time()
    with no_grad():
        for idx, image in enumerate(dataloaders):
            image = image.to(device)
            preds = model_he(image)    

            save_image(preds, 'HE.png')
            break

    end = time() - start
    print('he end {}m {}s'.format(end//60, end%60))

    start = time()
    with no_grad():
        for idx, image in enumerate(dataloaders):
            image = image.to(device)
            preds = model_ma(image)    

            save_image(preds, 'MA.png')
            break

    end = time() - start
    print('ma end {}m {}s'.format(end//60, end%60))
